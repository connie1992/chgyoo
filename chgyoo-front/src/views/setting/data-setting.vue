...
<template>
    <div>
        ...
        <Toolbar :btn-list="btnList" @add="add" @edit="edit" @delete="del" @set-status="setStatus"></Toolbar>
            ...
            <Form ref="dataForm" :rules="rules" :model="formItem" label-position="left" :label-width="80">
                ...
                <FormItem label="权限人" prop="user1Name">
                    <Input v-model="formItem.user1Name">
                        <SvgIconBtn icon-text="ren" btn-text="" type="text" height="35px" slot="suffix" btn-name="select-user" @on-select-user="selectUser"></SvgIconBtn>
                    </Input>
                </FormItem>
  ...
  export default {
    ...
    methods: {
      ...
      setStatus() {
        ...
        this.$Modal.confirm({
          title: '确认？',
          content: `确认将选中选设置为 ${check[0].status === '0' ? '生效' : '失效'} 吗？`,
          onOk: () => {
