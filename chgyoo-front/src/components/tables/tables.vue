...
<template>
    <div>
        <div id="tableID">
            ...
            <!-- 分页 -->
            <div v-if="showPage" style="padding-top: 8px;" ref="pagerDiv">
                <Row>
                    <Col span="18">
                        <Page show-sizer :show-elevator="showElevator" :page-size-opts="pagerPageSizeOpts" size="small"
                              ...
                              @on-change="changePage" @on-page-size-change="changePageSize" style="float: left;"/>
                        <span id="refreshBtn" style="display: inline-block;margin-left: 8px"><Button icon=" iconfont icon-shuaxin1" size="small" @click="refresh"></Button></span>
                    </Col>
  ...
  export default {
    ...
    watch: {
      propData: function (newList) {
        // 重置已选择的行数据
        this.selects = [];
        this.tableData = this.$util.copyObject(newList);
        this.tableData.forEach((item) => {
          this.$set(item, '_checked', false);
        });
      },
      '$store.state.user.currentBook': function () {
    ...
    methods: {
      ...
      getParams() {
        ...
        if (this.refreshByChangeBook) {
          params.book = this.$store.state.user.currentBook;
        }
        return params;
      },
      search() {
